...
public class DatabaseMigration implements CommandLineRunner {
    ...
    public void run(String... args) {
        ...
        dropFolderPermissionLevelCheckConstraint();
    ...
    /**
     * Drop the CHECK constraint on folder_permissions.permission_level that was
     * auto-generated by Hibernate when the column was an @Enumerated type.
     * The column now stores comma-separated values (e.g. "VIEW,UPLOAD,MANAGE")
     * which the old single-value check constraint rejects.
     */
    private void dropFolderPermissionLevelCheckConstraint() {
        try {
            String sql = """
                SELECT con.conname AS constraint_name
                FROM pg_constraint con
                JOIN pg_class rel ON rel.oid = con.conrelid
                JOIN pg_namespace nsp ON nsp.oid = rel.relnamespace
                WHERE rel.relname = 'folder_permissions'
                  AND con.contype = 'c'
                  AND nsp.nspname = 'public'
                """;
            ...
            for (Map<String, Object> row : constraints) {
                String constraintName = (String) row.get("constraint_name");
                if (constraintName.contains("permission_level")) {
                    log.info("Dropping stale CHECK constraint '{}' on folder_permissions (was enum-based, now comma-separated)",
                            constraintName);
                    jdbc.execute("ALTER TABLE folder_permissions DROP CONSTRAINT " + constraintName);
                    log.info("Successfully dropped constraint '{}'", constraintName);
                }
            }
        } catch (Exception e) {
            log.warn("Migration check for folder_permissions constraint: {}", e.getMessage());
        }
    }
}
