...
import type { StockTransferDto, CreateStockTransferRequest } from "../services/stockTransferService";
...
const STATUS_LIST = ["All", "DRAFT", "SUBMITTED", "APPROVED", "REJECTED", "COMPLETED"] as const;
...
const TRANSFER_TYPES = ["SITE_TO_SITE", "SITE_TO_COMPANY"] as const;
...
const emptyLineItem: LineItem = { materialId: 0, transferQty: 0, remarks: "" };
...
const emptyForm: TransferForm = {
  fromProjectId: 0, toProjectId: 0, transferDate: "",
  transferType: "SITE_TO_SITE", remarks: "",
  items: [{ ...emptyLineItem }],
};

export default function StockTransferPage() {
  const [records, setRecords] = useState<StockTransferDto[]>([]);
  ...
  const [form, setForm] = useState<TransferForm>({ ...emptyForm, items: [{ ...emptyLineItem }] });

  useEffect(() => { loadData(); loadDropdowns(); }, []);

  const loadData = async () => {
    setLoading(true);
    try { setRecords(await stockTransferService.getAll()); } catch (err) { console.error(err); setRecords([]); }
    setLoading(false);
  };
  ...
  const filtered = records.filter((r) => {
    ...
    if (projectFilter && r.fromProjectId !== projectFilter) return false;
    return true;
  });

  const openCreate = () => { setForm({ ...emptyForm, items: [{ ...emptyLineItem }] }); setShowModal(true); };

  const updateField = (field: keyof Omit<TransferForm, "items">, value: string | number) => {
  ...
  const handleSave = async () => {
    if (!form.fromProjectId) { alert("Please select From Project."); return; }
    if (!form.toProjectId) { alert("Please select To Project."); return; }
    if (form.fromProjectId === form.toProjectId) { alert("From and To projects must be different."); return; }
    if (!form.transferDate) { alert("Transfer Date is required."); return; }
    const validItems = form.items.filter((it) => it.materialId > 0 && it.transferQty > 0);
    ...
    try {
      const req: CreateStockTransferRequest = {
        ...
        transferType: form.transferType as "SITE_TO_SITE" | "SITE_TO_COMPANY",
      ...
      await stockTransferService.create(req);
      setShowModal(false);
      loadData();
    } catch (err: any) {
  ...
  const handleStatusChange = async (id: number, newStatus: string) => {
    try { await stockTransferService.updateStatus(id, newStatus); loadData(); }
    catch (err: any) { alert("Failed to update status.\n" + (err.message || err)); }
  };

  const handleDelete = async (id: number) => {
    if (!confirm("Are you sure you want to delete this transfer?")) return;
    try { await stockTransferService.delete(id); loadData(); }
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={openCreate} className="px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition">+ New Transfer</button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
        <div className="flex flex-wrap items-end gap-3">
          ...
          <select value={projectFilter} onChange={(e) => setProjectFilter(Number(e.target.value))} className="px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
            <option value={0}>All From Projects</option>
            {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        </div>
      </div>

      <p className="text-sm text-gray-500 mb-3">{filtered.length} transfer{filtered.length !== 1 ? "s" : ""} found</p>

      {loading ? (
        <div className="text-center py-10 text-gray-400">Loading...</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-10 text-gray-400">No stock transfers found.</div>
      ) : (
            ...
            <div key={r.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="flex flex-wrap items-center gap-4 px-5 py-4 cursor-pointer hover:bg-gray-50 transition" onClick={() => setExpandedId(expandedId === r.id ? null : r.id)}>
                <div className="flex-1 min-w-[140px]">
                  <p className="text-sm font-bold text-gray-800">{r.transferNo}</p>
                  <p className="text-xs text-gray-400">{r.transferType?.replace(/_/g, " ")}</p>
                </div>
                <div className="w-36">
                  <p className="text-xs text-gray-400">From</p>
                  <p className="text-sm text-gray-700">{r.fromProjectName}</p>
                </div>
                <div className="w-36">
                  <p className="text-xs text-gray-400">To</p>
                  <p className="text-sm text-gray-700">{r.toProjectName}</p>
                </div>
                <div className="w-28">
                  <p className="text-xs text-gray-400">Date</p>
                  <p className="text-sm text-gray-700">{r.transferDate}</p>
                </div>
                <div className="w-24">
                      ...
                      <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                        <thead className="bg-gray-100">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-right text-xs font-semibold text-gray-600">Transfer Qty</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto mx-4">
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-800">Create Stock Transfer</h2>
            </div>
            <div className="px-6 py-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">From Project <span className="text-red-500">*</span></label>
                  <select value={form.fromProjectId} onChange={(e) => updateField("fromProjectId", Number(e.target.value))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                    <option value={0}>-- Select --</option>
                    {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">To Project <span className="text-red-500">*</span></label>
                  <select value={form.toProjectId} onChange={(e) => updateField("toProjectId", Number(e.target.value))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                    <option value={0}>-- Select --</option>
                    {projects.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Transfer Date <span className="text-red-500">*</span></label>
                  <input type="date" value={form.transferDate} onChange={(e) => updateField("transferDate", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                </div>
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Transfer Type</label>
                  <select value={form.transferType} onChange={(e) => updateField("transferType", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                  ...
                  <table className="w-full text-sm border border-gray-200 rounded-lg">
                    <thead className="bg-gray-100">
                      <tr>
                        ...
                        <th className="px-2 py-2 text-right text-xs font-semibold text-gray-600 w-[120px]">Transfer Qty</th>
                    ...
                    <tbody>
                      {form.items.map((item, idx) => (
                        <tr key={idx} className="border-t border-gray-100">
                          ...
                          <td className="px-2 py-1.5">
                            <input type="number" min={0} value={item.transferQty || ""} onChange={(e) => updateItem(idx, "transferQty", Number(e.target.value))} className="w-full px-2 py-1.5 border border-gray-300 rounded text-xs text-right" />
                          </td>
                          <td className="px-2 py-1.5">
