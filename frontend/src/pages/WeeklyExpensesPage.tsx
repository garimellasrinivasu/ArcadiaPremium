...
const DEFAULT_NOTES =
  "Weekly recurring expenses for the project site including food, snacks, beverages, cleaning supplies, and other operational costs. Below is the detailed statement of expenses incurred, submitted for review and approval.";
...
export default function WeeklyExpensesPage() {
  ...
  async function loadExpenses() {
    try {
      setLoading(true);
      const data = await pujaExpensesService.getByPujaName("Weekly Expenses");
      setExpenses(data);
    } catch {
      // silently fail
    } finally {
...
<head><meta charset="utf-8"/><style>td,th{border:1px solid #999;padding:6px 10px;font-family:Arial}th{background:#14532d;color:#fff;font-weight:bold}.title{font-size:18px;font-weight:bold;color:#14532d}.sub{font-size:14px;color:#555}.total{font-weight:bold;background:#DCFCE7}.right{text-align:right}</style></head><body>`;
    html += `<table>`;
    html += `<tr><td colspan="9" class="title">Weekly Site Expenses - Expense Statement</td></tr>`;
    ...
    a.href = url;
    a.download = "Weekly_Expenses_Statement.xls";
  ...
  function handlePrint() {
    const tableRows = expenses
      .map(
        (exp, i) => `
        <tr style="border-bottom:1px solid #e5e7eb;${i % 2 === 1 ? "background:#f0fdf4;" : ""}">
          ...
          <td style="padding:8px;text-align:center;">${(!exp.paymentStatus || exp.paymentStatus === "Paid") ? '<span style="color:green;font-weight:600;">Paid</span>' : '<span style="color:red;font-weight:600;">Pending</span>'}</td>
          <td style="padding:8px;">${(!exp.paymentStatus || exp.paymentStatus === "Paid") ? (exp.paidBy || "") : (exp.payeeName ? "Pay to: " + exp.payeeName : "Payment Pending")}</td>
          <td style="padding:8px;">${(!exp.paymentStatus || exp.paymentStatus === "Paid") ? (exp.paymentMode || "-") : "-"}</td>
          <td style="padding:8px;">${(!exp.paymentStatus || exp.paymentStatus === "Paid") ? (exp.receiptNo || "-") : "-"}</td>
        </tr>`
      )
      .join("");
    ...
    const categorySummaryHTML =
      categoryEntries.length > 1
        ? `<h3 style="margin-top:24px;color:#14532d;">Category Summary</h3>
    ...
    const signaturesHTML = selectedPartners
      .map(
        ...
        <div style="display:inline-block;width:30%;text-align:center;margin:20px 1%;">
          <div style="border-top:2px solid #14532d;padding-top:8px;margin-top:60px;">
            <strong>${p}</strong><br/>
    ...
    const html = `
      <html><head><title>Weekly Site Expenses Statement</title></head>
      <body style="font-family:Arial,sans-serif;padding:40px;color:#333;">
        <div style="text-align:center;margin-bottom:24px;">
          <h1 style="color:#14532d;margin-bottom:4px;">Weekly Site Expenses - Expense Statement</h1>
          <h3 style="color:#166534;">Praneeth Pranav Arcadia Premium</h3>
        </div>
        ...
        <div style="background:#f0fdf4;padding:12px 16px;border-left:4px solid #16a34a;margin-bottom:24px;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          ...
          <tbody>
            ${tableRows}
            <tr style="border-top:3px solid #14532d;font-weight:bold;background:#dcfce7;">
        ...
        <div style="margin-top:40px;">
          <h3 style="color:#14532d;">Approved By</h3>
          ${signaturesHTML}
        </div>
      </body></html>`;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {/* ── Header ── */}
      <div className="bg-gradient-to-r from-green-900 via-green-800 to-green-900 text-white rounded-xl p-6 text-center shadow-lg">
        <h1 className="text-2xl md:text-3xl font-bold tracking-wide">
          Weekly Site Expenses
        </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-lg shadow p-4 space-y-3">
          <div>
            ...
            <input
              className="w-full border border-green-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-400 focus:outline-none"
              value={projectName}
        ...
        <div className="bg-gradient-to-br from-green-900 to-green-800 text-white rounded-lg shadow p-6 flex flex-col items-center justify-center">
          <span className="text-sm uppercase tracking-widest text-green-300 mb-1">
            Total Expenses
          </span>
      ...
      <div className="bg-green-50 border border-green-200 rounded-lg shadow p-4">
        ...
        <textarea
          className="w-full border border-green-300 bg-green-50 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-green-400 focus:outline-none min-h-[100px]"
          value={notes}
      ...
      <div ref={formRef} className="bg-white rounded-lg shadow p-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-green-100 text-green-900 rounded-lg hover:bg-green-200 font-bold text-lg leading-none"
              ...
              {showNewCategory && (
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-green-900 text-white rounded-lg hover:bg-green-800 text-sm font-medium"
                  >
                    Add
                  </button>
                </div>
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              className="px-6 py-2.5 bg-green-900 text-white rounded-lg hover:bg-green-800 font-semibold text-sm shadow"
            >
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="p-4 border-b border-green-200 flex items-center justify-between">
          <h2 className="text-lg font-bold text-green-900">Expense Details</h2>
          <div className="flex gap-2">
            <button
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {/* Total Row */}
                <tr className="bg-green-100 border-t-4 border-green-900 font-bold">
                  <td colSpan={6} className="px-4 py-3 text-right text-green-900 text-base">
                    Grand Total:
                  </td>
                  <td className="px-4 py-3 text-right font-mono text-green-900 text-base">
                    {formatINR(totalExpenses)}
                  </td>
                  <td colSpan={3}></td>
                </tr>
      ...
      {categoryEntries.length > 1 && (
        <div className="bg-white rounded-lg shadow p-5">
          <h2 className="text-lg font-bold text-green-900 mb-3">Category Summary</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {categoryEntries.map(([cat, total]) => (
              ...
              >
                ...
                <span className="text-sm font-bold text-green-900 font-mono whitespace-nowrap">
                  {formatINR(total)}
                </span>
              </div>
            ))}
      ...
      <div className="bg-white rounded-lg shadow p-5">
        <h2 className="text-lg font-bold text-green-900 mb-2">Partner Approval</h2>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition border ${
                  isSelected
                    ...
                    : "bg-white text-gray-600 border-gray-300 hover:border-green-400 hover:text-green-900"
                } ${
        ...
        {selectedPartners.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-green-200">
            {selectedPartners.map((partner) => (
              <div key={partner} className="text-center">
                <div className="h-20 border-b-2 border-green-900 mb-2"></div>
                <p className="font-semibold text-green-900">{partner}</p>
