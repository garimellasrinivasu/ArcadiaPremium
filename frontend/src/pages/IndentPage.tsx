...
import type { MaterialIndentDto, CreateIndentRequest } from "../services/indentService";
...
import type { MaterialRequisitionDto } from "../services/requisitionService";
...
const emptyLineItem: LineItem = { materialId: 0, indentQuantity: 0, remarks: "" };
...
export default function IndentPage() {
  const [records, setRecords] = useState<MaterialIndentDto[]>([]);
  ...
  const [requisitions, setRequisitions] = useState<MaterialRequisitionDto[]>([]);
  const [form, setForm] = useState<IndentForm>({ ...emptyForm, items: [{ ...emptyLineItem }] });
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const data = await indentService.getAll();
      setRecords(data);
    } catch (err) {
  ...
  const loadDropdowns = async () => {
    try {
      ...
      setProjects(p);
      setMaterials(m);
      setRequisitions(r.filter((req) => req.status === "APPROVED"));
    } catch (err) {
  ...
  const updateField = (field: keyof Omit<IndentForm, "items">, value: string | number) => {
  ...
  const handleSave = async () => {
    ...
    if (!form.indentDate) { alert("Indent Date is required."); return; }
    const validItems = form.items.filter((it) => it.materialId > 0 && it.indentQuantity > 0);
    if (validItems.length === 0) { alert("At least one valid item is required."); return; }

    setSaving(true);
    try {
      const req: CreateIndentRequest = {
        projectId: form.projectId,
        indentDate: form.indentDate,
        indentType: form.referenceType as "DIRECT" | "FROM_REQUISITION",
        requisitionId: form.referenceType === "FROM_REQUISITION" ? form.requisitionId : undefined,
        remarks: form.remarks || undefined,
        items: validItems.map((it) => ({
          materialId: it.materialId,
          indentQuantity: it.indentQuantity,
          remarks: it.remarks || undefined,
        })),
      };
      await indentService.create(req);
      setShowModal(false);
      loadData();
    } catch (err: any) {
  ...
  const handleStatusChange = async (id: number, newStatus: string) => {
    try {
      await indentService.updateStatus(id, newStatus);
      loadData();
    } catch (err: any) {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-800">Material Indents</h1>
        <button onClick={openCreate} className="px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition">+ New Indent</button>
      </div>
      ...
      <p className="text-sm text-gray-500 mb-3">{filtered.length} indent{filtered.length !== 1 ? "s" : ""} found</p>

      {loading ? (
        <div className="text-center py-10 text-gray-400">Loading...</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-10 text-gray-400">No indents found.</div>
      ) : (
            ...
            <div key={r.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              <div className="flex flex-wrap items-center gap-4 px-5 py-4 cursor-pointer hover:bg-gray-50 transition" onClick={() => setExpandedId(expandedId === r.id ? null : r.id)}>
                <div className="flex-1 min-w-[140px]">
                  <p className="text-sm font-bold text-gray-800">{r.indentNo}</p>
                  <p className="text-xs text-gray-400">{r.projectName}</p>
                </div>
                <div className="w-28">
                  <p className="text-xs text-gray-400">Date</p>
                  <p className="text-sm text-gray-700">{r.indentDate}</p>
                </div>
                <div className="w-36">
                  <p className="text-xs text-gray-400">Type</p>
                  <p className="text-sm text-gray-700">{r.indentType === "FROM_REQUISITION" ? "From Requisition" : "Direct"}</p>
                </div>
                <div className="w-20">
              ...
              {expandedId === r.id && (
                <div className="border-t border-gray-100 px-5 py-4 bg-gray-50">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mb-4">
                    {r.requisitionNo && (
                      <div>
                        <span className="text-gray-400 text-xs">Requisition</span>
                        <p className="font-semibold">{r.requisitionNo}</p>
                      </div>
                    )}
                    <div>
                      <span className="text-gray-400 text-xs">Created By</span>
                      <p className="font-semibold">{r.createdBy || "—"}</p>
                    </div>
                  </div>
                      ...
                      <table className="w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                        <thead className="bg-gray-100">
                          <tr>
                            ...
                            <th className="px-3 py-2 text-right text-xs font-semibold text-gray-600">Indent Qty</th>
                        ...
                        <tbody>
                          {r.items.map((item, idx) => (
                            <tr key={item.id ?? idx} className="border-t border-gray-100">
                              ...
                              <td className="px-3 py-2 text-right">{item.indentQuantity}</td>
                              <td className="px-3 py-2 text-right">{item.poQuantity ?? "—"}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-3xl max-h-[90vh] overflow-y-auto mx-4">
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="text-lg font-bold text-gray-800">Create Indent</h2>
            </div>
            <div className="px-6 py-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Indent Date <span className="text-red-500">*</span></label>
                  <input type="date" value={form.indentDate} onChange={(e) => updateField("indentDate", e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
                </div>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-gray-500 mb-1">Indent Type</label>
                  <div className="flex gap-4 mt-1">
                    <label className="flex items-center gap-1.5 text-sm">
                      <input type="radio" value="DIRECT" checked={form.referenceType === "DIRECT"} onChange={(e) => updateField("referenceType", e.target.value)} />
                      Direct
                    </label>
                    <label className="flex items-center gap-1.5 text-sm">
                      <input type="radio" value="FROM_REQUISITION" checked={form.referenceType === "FROM_REQUISITION"} onChange={(e) => updateField("referenceType", e.target.value)} />
                      From Requisition
                    </label>
                  </div>
                </div>
                {form.referenceType === "FROM_REQUISITION" && (
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Requisition</label>
                    <select value={form.requisitionId} onChange={(e) => updateField("requisitionId", Number(e.target.value))} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">
                      <option value={0}>-- Select Requisition --</option>
                      {requisitions.filter((rq) => !form.projectId || rq.projectId === form.projectId).map((rq) => (
                        <option key={rq.id} value={rq.id}>{rq.requisitionNo} - {rq.projectName}</option>
                      ))}
                    </select>
                  </div>
                )}
                  ...
                  <table className="w-full text-sm border border-gray-200 rounded-lg">
                    <thead className="bg-gray-100">
                      <tr>
                        ...
                        <th className="px-2 py-2 text-right text-xs font-semibold text-gray-600 w-[120px]">Indent Qty</th>
                    ...
                    <tbody>
                      {form.items.map((item, idx) => (
                        <tr key={idx} className="border-t border-gray-100">
                          ...
                          <td className="px-2 py-1.5">
                            <input type="number" min={0} value={item.indentQuantity || ""} onChange={(e) => updateItem(idx, "indentQuantity", Number(e.target.value))} className="w-full px-2 py-1.5 border border-gray-300 rounded text-xs text-right" />
                          </td>
                          <td className="px-2 py-1.5">
