...
const DEFAULT_CATEGORIES = [
  ...
  "Priest / Pandit Dakshina",
  "Flower Decorations & Garlands",
  ...
  "Chairs & Furniture Rental",
  "Venue Decoration & Lighting",
  "Gifts / Return Gifts",
  "Labour / Helper Charges",
...
const DEFAULT_NOTES =
  "The Shankustaphana (Foundation Ceremony) Puja was performed on 25th June 2026 with the divine blessings of HH Sri Tridandi Srimannarayana Ramanuja Chinna Jeeyar Swamiji and in the divine presence of HH Sri Tridandi Ahobila Ramanuja Jeeyar Swamiji. Below is the detailed statement of expenses incurred for the ceremony, submitted for review and approval by the partners.";
...
export default function PujaExpensesPage() {
  ...
  const [pujaDate, setPujaDate] = useState("2026-06-25");
  ...
  async function loadExpenses() {
    try {
      setLoading(true);
      const data = await pujaExpensesService.getAll();
      setExpenses(data);
    } catch {
      // silently fail
    } finally {
...
<head><meta charset="utf-8"/><style>td,th{border:1px solid #999;padding:6px 10px;font-family:Arial}th{background:#7B3F00;color:#fff;font-weight:bold}.title{font-size:18px;font-weight:bold;color:#7B3F00}.sub{font-size:14px;color:#555}.total{font-weight:bold;background:#FFF3CD}.right{text-align:right}</style></head><body>`;
    html += `<table>`;
    html += `<tr><td colspan="9" class="title">Shankustaphana Puja - Expense Statement</td></tr>`;
    html += `<tr><td colspan="9" class="sub">Praneeth Pranav Arcadia Premium</td></tr>`;
    html += `<tr><td colspan="9"></td></tr>`;
    html += `<tr><td><b>Project Name:</b></td><td colspan="8">${projectName}</td></tr>`;
    html += `<tr><td><b>Date of Puja:</b></td><td colspan="8">${formatDateDisplay(pujaDate)}</td></tr>`;
    html += `<tr><td><b>Prepared By:</b></td><td colspan="8">${preparedBy}</td></tr>`;
    html += `<tr><td colspan="9"></td></tr>`;
    ...
    if (categoryEntries.length > 1) {
      ...
      html += `<tr><th>Category</th><th class="right">Total</th><th colspan="7"></th></tr>`;
      categoryEntries.forEach(([cat, total]) => {
        html += `<tr><td>${cat}</td><td class="right">${total}</td><td colspan="7"></td></tr>`;
      });
      html += `<tr><td colspan="9"></td></tr>`;
    }
    ...
    selectedPartners.forEach((p) => {
      html += `<tr><td colspan="4">${p}</td><td colspan="5">Signature: ____________________</td></tr>`;
    });
    ...
    a.href = url;
    a.download = "Puja_Expenses_Statement.xls";
  ...
  async function handleImportExcel(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // Find the header row (S.No, Category, Description, ...)
      let headerIdx = -1;
      ...
      // Parse data rows after header until Total row
      const dataRows: CreatePujaExpenseRequest[] = [];
      for (let i = headerIdx + 1; i < rows.length; i++) {
        const cells = rows[i].querySelectorAll("td, th");
        if (cells.length < 5) continue;
        const firstCell = cells[0]?.textContent?.trim().toLowerCase() || "";
        // Stop at Total row or empty S.No
  ...
  function handlePrint() {
    const tableRows = expenses
      .map(
        (exp, i) => `
        <tr style="border-bottom:1px solid #e5e7eb;${i % 2 === 1 ? "background:#fefce8;" : ""}">
          ...
          <td style="padding:8px;">
            ${(!exp.paymentStatus || exp.paymentStatus === "Paid")
              ? (exp.paidBy || "")
              : (exp.payeeName ? "Pay to: " + exp.payeeName : "Payment Pending")}
          </td>
          <td style="padding:8px;">
    ...
    const categorySummaryHTML =
      categoryEntries.length > 1
        ? `<h3 style="margin-top:24px;color:#78350f;">Category Summary</h3>
    ...
    const signaturesHTML = selectedPartners
      .map(
        ...
        <div style="display:inline-block;width:30%;text-align:center;margin:20px 1%;">
          <div style="border-top:2px solid #78350f;padding-top:8px;margin-top:60px;">
            <strong>${p}</strong><br/>
    ...
    const html = `
      <html><head><title>Puja Expenses Statement</title></head>
      <body style="font-family:Arial,sans-serif;padding:40px;color:#333;">
        <div style="text-align:center;margin-bottom:24px;">
          <h1 style="color:#78350f;margin-bottom:4px;">Shankustaphana Puja - Expense Statement</h1>
          <h3 style="color:#92400e;">Praneeth Pranav Arcadia Premium</h3>
        </div>
        <div style="margin-bottom:16px;">
          <p><strong>Project Name:</strong> ${projectName}</p>
          <p><strong>Date of Puja:</strong> ${formatDateDisplay(pujaDate)}</p>
          <p><strong>Prepared By:</strong> ${preparedBy}</p>
        </div>
        <div style="background:#fefce8;padding:12px 16px;border-left:4px solid #ca8a04;margin-bottom:24px;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          <thead>
            <tr style="background:#78350f;color:white;">
              ...
              <th style="padding:8px;text-align:left;">Paid By / Payee</th>
          ...
          <tbody>
            ${tableRows}
            <tr style="border-top:3px solid #78350f;font-weight:bold;background:#fef3c7;">
              <td colspan="6" style="padding:10px;text-align:right;font-size:15px;">Grand Total:</td>
        ...
        <div style="margin-top:40px;">
          <h3 style="color:#78350f;">Approved By</h3>
          ${signaturesHTML}
        </div>
      </body></html>`;
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* ── Back Link ── */}
      <a href="/activities/expenses" className="inline-flex items-center gap-1 text-amber-700 hover:text-amber-900 text-sm font-medium transition">
        <span>&#8592;</span> Back to Expenses Summary
      </a>

      {/* ── Header ── */}
      <div className="bg-gradient-to-r from-amber-900 via-amber-800 to-amber-900 text-white rounded-xl p-6 text-center shadow-lg">
        <h1 className="text-2xl md:text-3xl font-bold tracking-wide">
          Shankustaphana Puja - Expense Statement
        </h1>
        <p className="text-amber-200 mt-1 text-sm md:text-base">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-white rounded-lg shadow p-4 space-y-3">
          <div>
            ...
            <input
              className="w-full border border-amber-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none"
              value={projectName}
        ...
        <div className="bg-gradient-to-br from-amber-900 to-amber-800 text-white rounded-lg shadow p-6 flex flex-col items-center justify-center">
          <span className="text-sm uppercase tracking-widest text-amber-300 mb-1">
            Total Expenses
          </span>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg shadow p-4">
        ...
        <textarea
          className="w-full border border-yellow-300 bg-yellow-50 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-amber-400 focus:outline-none min-h-[100px]"
          value={notes}
      ...
      <div ref={formRef} className="bg-white rounded-lg shadow p-5">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-amber-100 text-amber-900 rounded-lg hover:bg-amber-200 font-bold text-lg leading-none"
              ...
              {showNewCategory && (
                <div className="flex gap-2 mt-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-amber-900 text-white rounded-lg hover:bg-amber-800 text-sm font-medium"
                  >
                    Add
                  </button>
                </div>
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              className="px-6 py-2.5 bg-amber-900 text-white rounded-lg hover:bg-amber-800 font-semibold text-sm shadow"
            >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {/* Total Row */}
                <tr className="bg-amber-100 border-t-4 border-amber-900 font-bold">
                  <td colSpan={6} className="px-4 py-3 text-right text-amber-900 text-base">
                    Grand Total:
                  </td>
      ...
      {categoryEntries.length > 1 && (
        <div className="bg-white rounded-lg shadow p-5">
          <h2 className="text-lg font-bold text-amber-900 mb-3">Category Summary</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {categoryEntries.map(([cat, total]) => (
              ...
              >
                ...
                <span className="text-sm font-bold text-amber-900 font-mono whitespace-nowrap">
                  {formatINR(total)}
                </span>
              </div>
            ))}
      ...
      <div className="bg-white rounded-lg shadow p-5">
        <h2 className="text-lg font-bold text-amber-900 mb-2">Partner Approval</h2>
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm font-medium transition border ${
                  isSelected
                    ...
                    : "bg-white text-gray-600 border-gray-300 hover:border-amber-400 hover:text-amber-900"
                } ${
        ...
        {selectedPartners.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-amber-200">
            {selectedPartners.map((partner) => (
              <div key={partner} className="text-center">
                <div className="h-20 border-b-2 border-amber-900 mb-2"></div>
                <p className="font-semibold text-amber-900">{partner}</p>
