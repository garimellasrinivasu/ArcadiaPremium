...
export default function MBReportByActivityPage() {
  ...
  useEffect(() => {
    workOrderService.getAll().then(setWorkOrders).catch(() => {});
  }, []);
  ...
  const generateReport = async () => {
    ...
    setGenerated(true);
    try {
      const result = await subcontractingReportService.getMBReportByActivity(workOrderId);
      setData(result);
    } catch (err) {
  ...
  const totalQty = data.reduce((sum, r) => sum + (r.measuredQty ?? 0), 0);
  const totalAmount = data.reduce((sum, r) => sum + (r.measuredAmount ?? 0), 0);

  return (
    <div className="h-full flex flex-col">
      <h1 className="text-xl font-bold text-gray-800 mb-4">MB Report By Activity</h1>
      ...
      {/* Results */}
      {!generated ? (
        <div className="text-center py-10 text-gray-400">Select a work order and click Generate Report.</div>
      ) : loading ? (
        <div className="text-center py-10 text-gray-400">Loading...</div>
      ) : data.length === 0 ? (
        <div className="text-center py-10 text-gray-400">No measurement book data found for the selected work order.</div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Measured Qty</th>
                <th className="px-4 py-3 text-right text-xs font-semibold text-gray-500 uppercase">Measured Amount</th>
              </tr>
            </thead>
            <tbody>
              {data.map((r, idx) => (
                <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3 text-right text-gray-700">{(r.measuredQty ?? 0).toLocaleString("en-IN")}</td>
                  <td className="px-4 py-3 text-right font-semibold text-gray-800">{formatCurrency(r.measuredAmount ?? 0)}</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
